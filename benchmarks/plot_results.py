...
# Columns of the grid: runtime, memory
...
FUNCTION_ORDER = ["sort", "topk", "quantile", "max"]
...
def plot_method_comparison(df: pd.DataFrame, output_dir: Path, fmt: str):
    ...
    for category, cdf in df.groupby("category"):
        ...
        # rows = functions x 2 (fwd/grad), columns = metric types
        nrows = len(funcs) * 2
        ncols = len(col_specs)
        fig, axes = plt.subplots(
            nrows,
        ...
        for func_idx, func_name in enumerate(funcs):
            ...
            for col_idx, (_col_title, row_specs) in enumerate(col_specs):
                for phase_idx, (row_label, mean_col, _std_col) in enumerate(row_specs):
                    ax_row = func_idx * 2 + phase_idx
                    ax = axes[ax_row, col_idx]

                    methods = sorted(fdf[fdf["mode"] != "hard"]["method"].unique())
                    if not methods:
                        methods = [""]
                    ...
                    for m_idx, method in enumerate(methods):
                        vals = []
                        for mode in modes_present:
                            row = fdf[(fdf["mode"] == mode) & (fdf["method"] == method)]
                            vals.append(row[mean_col].values[0] if len(row) else 0)
